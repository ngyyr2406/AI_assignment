...
if page == "🔮 Review Prediction":
    ...
    st.dataframe(
        ...
        column_config={
            ...
            "Predicted Sentiment": st.column_config.TextColumn("Predicted Sentiment", width="medium"),
            "BERT": st.column_config.TextColumn("Model (BERT)", width="medium")
        }
    )
...
# ---- CSV Analysis Page ----
                ...
                **Common issues:**
                ...
                if "codec can't decode" in str(e) or "encoding" in str(e).lower():
                    ...
                    **Encoding Issue Detected:**
                    ...
                    # Show raw bytes for debugging (first 1000 bytes)
    ...
    if st.session_state.csv_results is not None and st.session_state.csv_analysis_done:
        ...
        # Create visualizations (Dashboard style) - excluding unknown for main charts
